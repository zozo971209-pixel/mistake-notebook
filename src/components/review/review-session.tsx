...
export function ReviewSession({ initialQuestions }: { initialQuestions: ReviewQuestion[] }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <div className="flex items-center justify-between text-sm text-muted-foreground"><span>{index + 1} / {questions.length}</span><span>還有 {remaining} 題</span></div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-5 rounded-xl bg-background/50 p-5 sm:p-7"><div className="whitespace-pre-wrap text-base leading-8">{question.question_text}</div><AnswerStructureDisplay config={answerConfig} /></div>
          ...
          {!revealed ? <Button className="w-full" size="lg" onClick={() => { if (!startedAt) setStartedAt(Date.now()); setRevealed(true); }}><Eye />確認答案</Button> : <div className="space-y-5">{structuredResult !== null && <Alert variant={structuredResult ? "default" : "destructive"}><AlertDescription className="font-medium">{structuredResult ? "答案比對正確。" : "答案不同，請查看正解。"}</AlertDescription></Alert>}<div className="rounded-xl border bg-muted/10 p-4"><p className="mb-3 text-sm font-medium text-primary">正確答案</p><AnswerStructureDisplay config={answerConfig} revealCorrect /></div><div className="grid gap-4 md:grid-cols-2"><Answer title="我的作答" text={answerConfig.kind === "fill_blank" ? blankValues.join("｜") : formatStructuredAnswer(answerConfig, selectedIds, answer)} /><Answer title="解題說明" text={[question.correct_answer, question.solution_text].filter(Boolean).join("\n\n")} /></div><div className="flex items-center gap-2"><Checkbox id="hint" checked={usedHint} onCheckedChange={(value) => setUsedHint(Boolean(value))} /><label htmlFor="hint" className="text-sm font-medium">這題有使用提示</label></div><p className="text-center text-sm font-medium">這題現在感覺如何？</p><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><GradeButton label="不會" icon={X} variant="destructive" onClick={() => grade("wrong")} disabled={busy} /><GradeButton label="有點難" icon={RotateCcw} variant="outline" onClick={() => grade("hard")} disabled={busy} /><GradeButton label="會了" icon={Check} variant="secondary" onClick={() => grade("correct")} disabled={busy} /><GradeButton label="很熟" icon={ChevronRight} variant="default" onClick={() => grade("easy")} disabled={busy} /></div></div>}
