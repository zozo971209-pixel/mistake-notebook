...
import { ArrowLeft, Bold, FileQuestion, Highlighter, Italic, List, PanelRightClose, PanelRightOpen, Plus, Save, Trash2 } from "lucide-react";
import { ColorPicker } from "@/components/outline/color-picker";
import { contentToEditorHtml, ResourceCard, resourceLabels, safeUrl, sanitizeRichHtml } from "@/components/outline/node-detail";
...
function SubjectEditor({ subject }: { subject: LocalSubject }) {
  ...
  useEffect(() => {
    if (editorRef.current) editorRef.current.innerHTML = contentToEditorHtml(subject.content ?? "");
  ...
  async function save() {
    ...
    await data.updateSubject(subject.id, { name: name.trim(), color, content: sanitizeRichHtml(editorRef.current?.innerHTML ?? ""), resources });
    setMessage("科目內容已儲存於這個瀏覽器。");
  }
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex gap-2"><Button variant="outline" onClick={() => setSidebarOpen((open) => !open)}>{sidebarOpen ? <PanelRightClose /> : <PanelRightOpen />}{sidebarOpen ? "專注閱讀" : "顯示側欄"}</Button><Button onClick={() => void save()}><Save />儲存科目</Button></div>
    </div>
      ...
      <Card className="overflow-hidden bg-muted/25 py-0">
        ...
        <div className="p-3 sm:p-8">
          <article className="mx-auto min-h-[72vh] max-w-[880px] bg-white px-6 py-8 text-slate-900 shadow-[0_10px_35px_rgb(31_41_55_/_0.10)] sm:px-12 sm:py-12">
            <div className="mb-8 border-b pb-5" style={{ borderColor: color }}><Badge variant="outline" style={{ borderColor: color, color }}>科目</Badge><h1 className="mt-3 text-3xl font-semibold tracking-tight">{name || "未命名科目"}</h1><p className="mt-2 text-sm text-slate-500">整理這個科目的全貌、學習方法與重要提醒。</p></div>
            <div ref={editorRef} contentEditable suppressContentEditableWarning role="textbox" aria-label="科目內容" aria-multiline="true" className="rich-editor min-h-[420px] outline-none" data-placeholder="輸入科目概覽、學習順序、核心觀念與提醒…" onInput={rememberSelection} onKeyUp={rememberSelection} onMouseUp={rememberSelection} />
