...
export async function prepareAiRequest(request: Request, action: "extract_question" | "tutor" | "similar") {
  ...
  if (!byok) {
    const { data: allowed, error } = await supabase.rpc("consume_ai_quota", { action_name: action, selected_model: DEFAULT_GEMINI_MODEL });
  ...
  return { supabase, userId, apiKey, model: DEFAULT_GEMINI_MODEL, byok: Boolean(byok) };
}
