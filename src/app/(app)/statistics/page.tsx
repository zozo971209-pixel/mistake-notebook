...
export default async function StatisticsPage() {
  ...
  return <div className="space-y-8"><PageHeader title="學習分析" description="看見進步，也找到下一個需要加強的地方。" />
    <section className="grid divide-y rounded-2xl border bg-card sm:grid-cols-3 sm:divide-x sm:divide-y-0"><Metric icon={CheckCircle2} label="近 30 日正確率" value={`${accuracy}%`} /><Metric icon={Target} label="平均熟練度" value={`${average}%`} /><Metric icon={Flame} label="已掌握" value={String(mastered)} /></section>
