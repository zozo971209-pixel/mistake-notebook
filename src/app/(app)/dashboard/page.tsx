...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid divide-y rounded-2xl border bg-card/40 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
      ...
      <section className="grid gap-8 lg:grid-cols-[1fr_320px]">
        <div><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold">最近錯題</h2><Button asChild variant="ghost" size="sm"><Link href="/questions">查看全部<ArrowRight /></Link></Button></div><div className="divide-y overflow-hidden rounded-2xl border bg-card/35">{!recent?.length && <Empty />}{recent?.map((question) => { const subject = Array.isArray(question.subjects) ? question.subjects[0] : question.subjects; return <Link key={question.id} href={`/questions/${question.id}`} className="flex items-center gap-4 p-4 transition hover:bg-accent/40"><span className="h-10 w-1 rounded-full" style={{ backgroundColor: subject?.color ?? "#8b5cf6" }} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2">{subject?.name && <Badge variant="secondary">{subject.name}</Badge>}<span className="truncate text-sm font-medium">{question.title || question.question_text}</span></div><p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{question.question_text}</p></div><span className="font-mono text-sm text-primary">{question.mastery_score}%</span></Link>; })}</div></div>
