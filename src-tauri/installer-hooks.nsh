!macro NSIS_HOOK_POSTINSTALL
  IfFileExists "$DESKTOP\學習地圖.lnk" 0 learning_map_desktop_done
    CreateShortcut "$DESKTOP\學習地圖.lnk" "$INSTDIR\mistake-notebook.exe" "" "$INSTDIR\learning-map-icon-0.1.37.ico" 0
  ...
  IfFileExists "$SMPROGRAMS\學習地圖.lnk" 0 learning_map_start_menu_done
    CreateShortcut "$SMPROGRAMS\學習地圖.lnk" "$INSTDIR\mistake-notebook.exe" "" "$INSTDIR\learning-map-icon-0.1.37.ico" 0
